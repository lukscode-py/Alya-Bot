...
export function formatLocalStatus(status) {
  return [
    ...
    `Modelo/runtime local: ${code(status.modelPath)}`,
    `API local: ${code(status.baseUrl)}`,
    `Auto iniciar servidor: ${yesNo(status.autoStartServer)}`,
    "",
    "*Ambiente*",
  ...
  return [
    ...
    ...models.map((model) =>
      [
        ...
        `Modelo Ollama: ${code(model.ollamaModel || model.id)}`,
